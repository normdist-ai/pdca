...
1. **初始化输出目录**：如果`.pdca/`目录不存在，创建目录并生成`README.md`说明文件（内容见下方"README.md内容规范"）
2. **获取当前时间**：使用 `Get-Date -Format "yyyyMMdd-HHmmss"` 获取实际当前时间
3. **创建问题文件夹**：创建 `issue-yyyyMMdd-HHmmss/` 文件夹（使用实际时间）
4. **创建问题概述**：在问题文件夹中创建 `README.md`，记录问题基本信息
5. **创建PDCA过程文件**：在相应阶段文件夹中创建过程文件
6. **更新问题索引**：在 `issue-list.md` 中添加或更新问题记录
7. **更新问题状态**：随着PDCA循环进展，及时更新问题状态

**重要提示**：
- ⚠️ 必须使用实际当前时间，不能使用估算时间
- ⚠️ 时间格式必须为 `yyyyMMdd-HHmmss`
- ⚠️ 创建文件夹后应验证时间是否正确
- ⚠️ 首次使用时必须初始化`.pdca/`目录并创建`README.md`

#### README.md内容规范

`.pdca/README.md`是输出目录的说明文件，在首次初始化`.pdca/`目录时自动生成，包含以下内容：
